...
/*   Updated: 2021/02/27 16:37:32 by ealexa           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	pthread_mutex_lock(&philo->mutex_arr[philo->rigth]);
	print_params(philo, FORK);
	pthread_mutex_lock(&philo->mutex_arr[philo->left]);
	print_params(philo, FORK);
}

void			return_fork(t_philo *philo)
{
	pthread_mutex_unlock(&philo->mutex_arr[philo->left]);
	pthread_mutex_unlock(&philo->mutex_arr[philo->rigth]);
}
